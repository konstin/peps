...
system, and use the index specifically for PyTorch. Users that install on
different platforms, such as CPU-only on developer machines and GPUs in
production, need to add special logic to select the correct index, and
be mindful that the environment changes and whether the installed packages
fit won't be detected by the packaging tool.
...
version segments and may need to add support for toggling between index URLs
in the same lockfile, while also supporting changing local version segments.
PyTorch breaks the pattern that packages are usually installed with through
these requirements, both for users and for tools. Problems with installing
Pytorch are a very common point of user confusion. To quantify this, in uv's
issue tracker 552/8136, or 6.8%, of issues contain the term "torch".
