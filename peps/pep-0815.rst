...
No major Python packaging tool supports generating or checking either ``RECORD.jws`` or
``RECORD.p7s``. Notably, neither pip nor uv validate the hashes in ``RECORD``,
a requirement for using signature files. The
